...
describe("synchronization crash recovery", () => {
  ...
  it("preserves the original schedule rollback when replaying a completed action", async () => {
    const value = fixture(temporaryDirectory());
    const base = persistedPlan(value);
    const schedule = {
      id: decode(ActionId)("action:canonfig.schedule-default:1:schedule-default"),
      resource: decode(ResourceId)("canonfig.schedule-default"),
      kind: "schedule-default" as const,
      detail: {
        kind: "schedule-default" as const,
        operation: "upsert" as const,
        schedule: { kind: "daily" as const, localTime: "03:30" },
      },
      before: [],
    };
    const body = {
    ...
    const reference = join(
      ...
      "rollback",
      "run-recovery",
      `${sha256Hex(schedule.id)}.schedule.json`,
    );
    ...
    writeFileSync(reference, JSON.stringify({
      ...
      servicePresent: true,
      schedulePresent: true,
      schedule: JSON.stringify(previous),
    }));
    await journal(value, schedule.id, "running", reference);
    await journal(value, schedule.id, "succeeded", reference);
    const scheduled = inMemoryScheduleManager(schedule.detail.schedule);
    const machine = decorateMachine(value.root, (service) => ({
      ...service,
      atomicWrite: (input) => input.path.absolute === value.target
        ? Effect.fail({
    ...
    const outcome = await recover(value, machine, scheduled.manager);

    expect(outcome.outcome).toBe("Failed");
    expect(scheduled.current()).toEqual(previous);
  });

  it.each([
    ["the schedule still verifies", "desired"],
    ["a crash restored the schedule before journaling", "previous"],
  ] as const)(
    "rolls back a completed schedule resource when later recovery fails: %s",
    async (_case, initialState) => {
    const value = fixture(temporaryDirectory());
    const base = persistedPlan(value);
    const scheduleId = decode(ResourceId)("schedule");
    const target = join(value.root, "schedule.json");
    const scheduleBytes = new TextEncoder().encode(JSON.stringify({
      kind: "schedule",
      calendar: { type: "daily", at: "03:30" },
      timezone: "local",
    }));
    const digest = decode(ContentDigest)(sha256BytesHex(scheduleBytes));
    const desiredSchedule = { kind: "daily" as const, localTime: "03:30" };
    ...
    const revision: PlanningProfileRevision = {
      ...
      }, ...value.revision.desired],
    };
    const body = {
      revision: base.revision,
      follower: base.follower,
      requiredBlobs: base.requiredBlobs,
      actions: [scheduleAction, ...base.actions],
      agentTasks: base.agentTasks,
    };
    const encoded = canonicalJson(Schema.decodeUnknownSync(Schema.MutableJson)(body));
    const plan = {
    ...
    const scheduledValue: Fixture = {
      ...value,
      revision,
      recovery: {
        follower: follower.id,
        revision,
        artifacts: [value.artifact, { digest, content: scheduleBytes }],
      },
    };
    const previousApplied = {
      resource: scheduleId,
      revision: revision.id,
      digest: decode(ContentDigest)(sha256Hex("previous schedule")),
      appliedAt: "2026-08-15T00:00:59Z",
    ...
    await seed(scheduledValue, plan);
    const reference = writeScheduleRollbackPayload(
      scheduledValue,
      scheduleAction.id,
      previousSchedule,
    );
    await journal(scheduledValue, scheduleAction.id, "running", reference);
    await journal(scheduledValue, scheduleAction.id, "succeeded", reference, {
      appliedResource: currentApplied,
      removedResourceRecord: previousApplied,
    });
    const scheduled = inMemoryScheduleManager(
      initialState === "desired" ? desiredSchedule : previousSchedule,
    );
    const machine = decorateMachine(value.root, (service) => ({
      ...service,
      atomicWrite: (input) => input.path.absolute === value.target
    ...
    const outcome = await recover(scheduledValue, machine, scheduled.manager);

    expect(outcome.outcome).toBe("Failed");
    expect(scheduled.current()).toEqual(previousSchedule);
    const applied = await Effect.runPromise(
      Effect.flatMap(StateRepository, (repository) =>
        repository.loadAppliedResources(follower.id)
      ).pipe(Effect.provide(stateRepositoryLayer(value.database))),
  ...
  it("restores a completed schedule removal without the removed artifact", async () => {
    const value = fixture(temporaryDirectory());
    const scheduleId = decode(ResourceId)("schedule");
    const scheduleTarget = join(value.root, "removed-schedule.json");
    const scheduleDigest = decode(ContentDigest)(sha256Hex("removed schedule artifact"));
    const previousSchedule = { kind: "daily" as const, localTime: "02:15" };
    const applied = {
      resource: scheduleId,
      revision: value.revision.id,
    ...
    const previousRevision: PlanningProfileRevision = {
      ...value.revision,
      resources: [
        scheduleResource,
        ...value.revision.resources.map((resource) => ({
          ...resource,
    ...
    const removedRevision: PlanningProfileRevision = {
      ...previousRevision,
      id: decode(ProfileRevisionId)("revision-recovery-schedule-removal"),
      sequence: 2,
      canonicalBytes: "{\"removedSchedule\":true}",
      digest: decode(ContentDigest)(sha256Hex("{\"removedSchedule\":true}")),
      removedResources: [scheduleId],
    };
    const planned = Effect.runSync(planSynchronization({
    ...
    const removal = planned.actions.find((action) => action.resource === scheduleId)!;
    expect(removal.detail).toMatchObject({
      kind: "remove-resource",
      schedule: previousSchedule,
    });
    const later = planned.actions.find((action) =>
      action.resource === value.revision.resources[0]!.id
    )!;
    expect(later.before).toContain(removal.id);
    const scheduledValue: Fixture = {
      ...value,
      revision: removedRevision,
      recovery: {
        follower: follower.id,
        revision: removedRevision,
        // Removed resources have no artifact in the current revision.
    ...
    await Effect.runPromise(
      Effect.gen(function*() {
        const repository = yield* StateRepository;
        yield* repository.registerFollower({ follower });
        yield* repository.publishRevision({ revision: previousRevision });
        yield* repository.publishRevision({
          revision: {
            ...removedRevision,
            resources: removedRevision.resources.filter((resource) =>
              resource.id !== scheduleId
            ),
          },
        });
        yield* repository.startRun({
    ...
    await journal(scheduledValue, removal.id, "running", reference);
    await journal(scheduledValue, removal.id, "succeeded", reference, {
      removedResource: scheduleId,
    ...
    expect(outcome.outcome).toBe("Failed");
    expect(runRows(scheduledValue)).toContainEqual(expect.objectContaining({
      action_id: later.id,
      state: "failed",
    }));
    expect(scheduled.current()).toEqual(previousSchedule);
  ...
  it.each([
    ["no-op", { kind: "no-op" as const }],
    [
      "verify-only",
      { kind: "verify-only" as const, method: "native-scheduler" },
    ],
  ] as const)(
    "recovers a completed schedule %s without native rollback material",
    async (actionKind, detail) => {
    const value = fixture(temporaryDirectory());
    const scheduleId = decode(ResourceId)("schedule");
    const target = join(value.root, "schedule.json");
    const scheduleBytes = new TextEncoder().encode(JSON.stringify({
    ...
    const plannedScheduleAction = planned.actions.find((action) =>
      action.resource === scheduleId
    )!;
    expect(plannedScheduleAction.detail.kind).toBe("no-op");
    const scheduleAction = actionKind === "no-op"
      ? plannedScheduleAction
      : {
        ...plannedScheduleAction,
        id: decode(ActionId)("action:schedule:0:verify-only"),
        kind: actionKind,
        detail,
      };
    const actions = planned.actions.map((action) =>
      action.resource === scheduleId ? scheduleAction : action
    );
    const body = {
      revision: planned.revision,
      follower: planned.follower,
      requiredBlobs: planned.requiredBlobs,
      actions,
      agentTasks: planned.agentTasks,
    };
    const encoded = canonicalJson(Schema.decodeUnknownSync(Schema.MutableJson)(body));
    const plan = {
      ...planned,
    ...
    await seed(scheduledValue, plan);
    await journal(scheduledValue, scheduleAction.id, "running");
    await journal(scheduledValue, scheduleAction.id, "succeeded", undefined, {
      appliedResource: {
        resource: scheduleId,
        revision: revision.id,
        digest,
    ...
    const scheduled = inMemoryScheduleManager(desiredSchedule);
    const machine = decorateMachine(value.root, (service) => ({
      ...service,
      atomicWrite: (input) => input.path.absolute === value.target
        ? Effect.fail({
    ...
    expect(outcome.outcome).toBe("Failed");
    expect(scheduled.current()).toEqual(desiredSchedule);
    const applied = await Effect.runPromise(
      Effect.flatMap(StateRepository, (repository) =>
        repository.loadAppliedResources(follower.id)
      ).pipe(Effect.provide(stateRepositoryLayer(value.database))),
  ...
  it("fails closed before reverifying a completed schedule mutation without rollback material", async () => {
    const value = fixture(temporaryDirectory());
    const scheduleId = decode(ResourceId)("schedule");
    const target = join(value.root, "schedule.json");
    const scheduleBytes = new TextEncoder().encode(JSON.stringify({
    ...
    const scheduleAction = plan.actions.find((action) =>
      action.resource === scheduleId
    )!;
    expect(scheduleAction.detail.kind).toBe("write-file");
    const scheduledValue: Fixture = {
      ...value,
      revision,
      recovery: {
    ...
    await expect(
      recover(scheduledValue, machineLayer(value.root), scheduled.manager),
    ).rejects.toMatchObject({
      _tag: "RecoveryIntegrityError",
      message: expect.stringContaining("lacks rollback material"),
    });
    const applied = await Effect.runPromise(
      Effect.flatMap(StateRepository, (repository) =>
        repository.loadAppliedResources(follower.id)
    ...
    expect(applied).toEqual([currentApplied]);
  });
