...
`--allow-leaf-executable` authorizes only a direct leaf operation.
`--allow-script-interpreter` authorizes only bounded script-file invocation.
...
`--allow-path`, either executable classification option, `--allow-origin`, or
