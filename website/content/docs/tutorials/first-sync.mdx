...
Keep this running. `--host` accepts only `127.0.0.1` or `::1`, so the shipped server is
reachable from the same machine only.

For a remote follower, Canonfig manages the approved SSH loopback tunnel after
you create the invitation below. You need the Source SSH host's verified public
key in a local file; Canonfig will not accept `StrictHostKeyChecking=no`,
`accept-new`, a null known-hosts file, or an additional forward.
...
```bash
canonfig source invite --endpoint https://127.0.0.1:17342 --output ./canonfig-invite --expires 15m
```

The mode-`0600` output is a bounded base64url envelope with an explicit EOF
marker. It contains the loopback endpoint, independent TLS and signing pins,
and a one-time token. Transfer the file through an authenticated private
channel; command output contains no invitation material.

## 5. Enroll the Follower Machine

On a remote Follower Machine, start the managed tunnel, then pipe the invitation
envelope into enrollment:
