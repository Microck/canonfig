...
To enroll a follower on another machine, forward this port to the follower's own loopback
address yourself, then leave the invitation endpoint as the loopback origin the follower
will dial. Run the forward on the follower machine:

```bash
ssh -N -L 17342:127.0.0.1:17342 <user>@<source-host>
```

The follower pins the source TLS certificate at enrollment, so the tunnel has to pass it
through unchanged. The cross-platform reference states the same limit: remote exposure and
public listeners are outside the shipped CLI contract.
